...
interface ChatPanelProps {
  ...
  onRestoreCheckpoint?: (code: { html: string; css: string; js: string }) => void;
    ...
    if (mode === 'chat') {
      ...
      return messages.filter((msg, idx) => {
        ...
        if (msg.sender === 'ai') {
          try {
            const parsed = JSON.parse(msg.content);
            return parsed && typeof parsed === 'object' && (parsed.html || parsed.css || parsed.js);
          } catch {
            return false;
          }
        }
              ...
              return (
                ...
                >
                  ...
                  <div
                    ...
                    } ${isStreamingMessage ? 'border-2 border-blue-400 dark:border-blue-500 shadow-lg' : ''}`}
                  >
                    {(() => {
                      if (isThinkBlock) {
                          ...
                          <>
                            {isGenerating && (index === lastStreamingMessageIndex) && !isJson ? (
                              ...
                              <div>
                                ...
                                {showCode && (
                                  <div className="mt-2 text-xs bg-gray-100 dark:bg-gray-900 rounded p-2 overflow-x-auto">
                                    <pre className="mb-1"><b>HTML:</b>\n{codeJson.html || ''}</pre>
                                    <pre className="mb-1"><b>CSS:</b>\n{codeJson.css || ''}</pre>
                                    <pre><b>JS:</b>\n{codeJson.js || ''}</pre>
                                  </div>
                                )}
                              </div>
